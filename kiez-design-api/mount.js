// Variante B: kiez-design-api als mountbarer Express-Router.
// Wird sowohl vom Standalone-server.js (Variante A) als auch direkt vom
// bestehenden trikot-konfigurator-Express-Prozess (Variante B) verwendet.
//
// Routen (absolut, damit public/index.html mit api='/api/kiez' 1:1 bleibt):
//   GET  /api/kiez/cities | /api/kiez/resolve | /api/kiez/job/:id | /api/kiez/preview/:file
//   POST /api/kiez/generate | /api/kiez/claim
//   POST /webhook/shopify/orders_paid
//   GET  /kiez/test/*  (Test-UI, statisch)
...
// Erzeugt den Kiez-Router. `root` = Ordner mit data/ und public/ (Default: CWD,
// beim Mount in trikot-konfigurator explizit auf .../kiez-design-api gesetzt).
export async function createKiezRouter(opts = {}) {
  const root = opts.root || process.env.KIEZ_ROOT || process.cwd();
  const env = {
    ...
    PUBLIC_BASE_URL: 'http://localhost:3100', PREVIEW_TTL_DAYS: 7, ...process.env, ...opts
  };
  ...
  const data = JSON.parse(await fs.readFile(path.join(root, 'data', 'cities.json')));
  const STORE = env.KIEZ_STORAGE_DIR ? path.resolve(env.KIEZ_STORAGE_DIR) : path.join(root, 'storage');
  ...
  const json = express.json({ limit: '64kb' }); // no-op falls Host-App den Body schon geparst hat (req._body)

  const proxyGuard = (req, res, next) => verifyProxy(req.query, env.SHOPIFY_APP_SECRET) ? next() : res.status(403).json({ error: 'bad signature' });

  router.get('/api/kiez/cities', proxyGuard, (req, res) => {
    res.json({
  ...
  router.get('/api/kiez/resolve', proxyGuard, async (req, res) => {
  ...
  router.post('/api/kiez/generate', json, proxyGuard, async (req, res) => {
  ...
  router.get('/api/kiez/job/:id', proxyGuard, (req, res) => {
  ...
  router.get('/api/kiez/preview/:file', async (req, res) => {
  ...
  router.post('/api/kiez/claim', json, proxyGuard, async (req, res) => {
  ...
  // Body kann Buffer (Standalone) oder bereits geparstes Objekt (Host-App mit globalem json) sein.
  router.post('/webhook/shopify/orders_paid', express.raw({ type: '*/*' }), async (req, res) => {
    const raw = Buffer.isBuffer(req.body) ? req.body : Buffer.from(JSON.stringify(req.body || {}));
    if (env.SHOPIFY_APP_SECRET) {
      const h = crypto.createHmac('sha256', env.SHOPIFY_APP_SECRET).update(raw).digest('base64');
      if (h !== req.get('X-Shopify-Hmac-Sha256')) return res.status(401).end();
    }
    const order = Buffer.isBuffer(req.body) ? JSON.parse(raw) : (req.body || {});
  ...
  // Test-UI unter /kiez/test/  (nutzt intern weiterhin /api/kiez/*)
  router.use('/kiez/test', express.static(path.join(root, 'public')));
