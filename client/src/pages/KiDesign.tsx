    ...
    try {
      ...
      if (useLandmark && landmarkSilhouette) {
        ...
        if (landmarkStyle === "large") {
          ...
          extraPrompt += ` [MANDATORY SILHOUETTE OVERLAY - ZERO CREATIVE FREEDOM]: You are provided a reference image that shows a SPECIFIC building/landmark silhouette. You MUST trace and reproduce THIS EXACT OUTLINE as a large watermark ${placementText}. The silhouette must cover approximately 50-60% of the jersey surface, centered. Render it as a single-color tonal shape (same hue as jersey, slightly lighter or darker) at ${landmarkOpacity}% opacity. STRICT RULES: 1) COPY the outline from the reference image PIXEL BY PIXEL - do NOT interpret, simplify, or replace it. 2) Do NOT use ANY other shape - no church, no tower, no bridge, no generic landmark, no trophy, no ball, no star, no shield. 3) If you cannot reproduce the exact shape, leave the area BLANK rather than substituting. 4) The silhouette is BEHIND all other elements (numbers, logos, text must be in front).`;
        } else {
          // Kleine zufällige als Muster
          extraPrompt += ` [MANDATORY SILHOUETTE PATTERN - ZERO CREATIVE FREEDOM]: You are provided a reference image that shows a SPECIFIC building/landmark silhouette. You MUST trace THIS EXACT OUTLINE and repeat it as a scattered pattern of small copies (2-5cm each) ${placementText}. Scatter many copies at various sizes and slight rotations across the jersey fabric. Render each copy as a single-color tonal shape (same hue as jersey, slightly lighter or darker) at ${landmarkOpacity}% opacity. STRICT RULES: 1) Every single shape in the pattern MUST be an exact copy of the silhouette from the reference image - PIXEL BY PIXEL reproduction. 2) Do NOT use ANY other shape - no generic icons, no trophies, no balls, no stars, no shields, no other buildings. 3) If you cannot reproduce the exact shape, leave the area BLANK rather than substituting. 4) The pattern is BEHIND all other elements (numbers, logos, text must be in front).`;
        }
      }
