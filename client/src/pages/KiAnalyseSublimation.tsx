...
import { Upload, Plus, Trash2, Save, Sparkles, Loader2, ChevronLeft, Maximize2, AlertTriangle, Layers } from "lucide-react";
...
export default function KiAnalyseSublimation() {
  ...
  useEffect(() => {
    ...
    const existing = document.getElementById("ki-sub-fonts");
  ...
  // ── KI-Analyse starten (Sublimation: kein Freistellen nötig) ──
  const handleAiAnalyze = async () => {
    if (!imageUrl) return;
    setAnalyzing(true);
    try {
      const fullImageUrl = imageUrl.startsWith("http") ? imageUrl : `${window.location.origin}${storageUrl(imageUrl) || imageUrl}`;

      // Zonen erkennen – bei Sublimation bleibt das Bild als Ganzes
      ...
      if (result.zones && result.zones.length > 0) {
        ...
        setZones(newZones);
        toast.success(`${newZones.length} Zonen erkannt – Sublimation: Ganzkörper-Design bleibt erhalten.`);
        setStep("edit");
      } else {
  ...
  const openFullscreenEditor = useCallback(() => {
    ...
    const displayImageUrl = imageUrl ? (imageUrl.startsWith("http") ? imageUrl : (storageUrl(imageUrl) || imageUrl)) : undefined;
    openEditor({
  ...
  }, [zones, orgId, selectedSport, openEditor, registerSaveCallback, registerDiscardCallback, setLocation, imageUrl]);
  ...
  const handleSave = async () => {
    ...
    try {
      const result = await createTemplate.mutateAsync({
        ...
        positionsConfig: zones.length > 0 ? { productId: null, zones } : undefined,
        orgId,
      ...
      toast.success("Sublimations-Vorlage gespeichert!");
  ...
  // ── Angezeigtes Bild ──
  const displayImage = imageUrl ? (storageUrl(imageUrl) || imageUrl) : null;
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="text-lg font-semibold">KI-Analyse – Sublimation</h1>
                <p className="text-xs text-muted-foreground">Ganzkörper-Sublimationsvorlage erstellen</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {!imageUrl ? (
              <div
                className="border-2 border-dashed border-blue-300 rounded-xl p-12 text-center cursor-pointer hover:border-blue-500 hover:bg-blue-50 transition-all bg-white"
                onClick={() => fileInputRef.current?.click()}
              >
                <Upload className="w-12 h-12 mx-auto mb-4 text-blue-400" />
                <p className="text-lg font-medium text-gray-700">
                  {uploading ? "Wird hochgeladen..." : "Sublimations-Trikot hochladen"}
                </p>
                <p className="text-sm text-muted-foreground mt-2">
                  Laden Sie ein Foto eines bestehenden Sublimations-Trikots hoch.
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  JPG, PNG, WebP – mind. 300 DPI empfohlen
                </p>
                <p className="text-xs text-blue-600 font-medium mt-3">
                  Sublimation: Das gesamte Design wird als Ganzkörper-Druck übernommen.
                </p>
                <input
                  ref={fileInputRef}
                  type="file"
            ...
            {step === "analyze" && imageUrl && (
              <div className="bg-white rounded-xl border p-6 text-center">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  Die KI erkennt automatisch alle Druckpositionen auf dem Sublimations-Trikot.
                  Das Bild bleibt als Ganzes erhalten (kein Freistellen).
                </p>
                <Button
                  size="lg"
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border p-4 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="z.B. Auswärtstrikot Sublimation 2025"
                />
              </div>
              <div className="space-y-2">
            ...
            <div className="bg-blue-50 rounded-xl border border-blue-200 p-4">
              <h3 className="font-semibold text-sm text-blue-800 mb-2">Sublimationsverfahren</h3>
              <ul className="text-xs text-blue-700 space-y-1">
                <li>• Ganzkörper-Druck (All-Over)</li>
                <li>• Design wird direkt in den Stoff sublimiert</li>
                <li>• Keine Freistelling nötig – Bild bleibt komplett</li>
                <li>• Ideal für komplexe Designs und Muster</li>
                <li>• Alle Teile (Vorder-/Rückteil, Ärmel, Kragen) konfigurierbar</li>
              </ul>
            </div>
          </div>
        </div>
